...
###Wake County voter registration.
Use this URL to set all correct data types: https://s3.amazonaws.com/dl.ncsbe.gov/data/layout_ncvoter_ncvhis.txt
```{r}
temp <- tempfile()

download.file("https://s3.amazonaws.com/dl.ncsbe.gov/data/ncvoter92.zip",temp)

voters_wake <- read_delim(unz(temp, "ncvoter92.txt"),
                            "\t", 
                            escape_double = FALSE, 
                            trim_ws = TRUE, 
...
url <- "https://cf.ncsbe.gov/CFOrgLkup/ExportDetailResults/?ReportID=171712&Type=REC&Title=COMMITTEE%20TO%20ELECT%20KIM%20COLEY%20-%202019%20Year%20End%20Semi-Annual"

#read_csv() does not do a great job here guessing the correct col_types, so we have to define them explicitly.

coley_rcpts_2019A <- read_csv(url, 
    col_types = cols(
...
names(coley_rcpts_2019A) <- c("date","prior","donor","street1","street2","city","state","zip","country","postal","profession","employer","purpose","type","account","payment_form","description","amount","sum_to_date")
